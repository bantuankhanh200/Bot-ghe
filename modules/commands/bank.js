...
module.exports.run = async function({ api, event, args, models, Users, Threads, Currencies, permssion }) {
  ...
  if (!user.find(i => i.senderID == senderID)) return api.sendMessage('[ 𝐂𝐀̉𝐍𝐇 𝐁𝐀́𝐎 ] » Bạn chưa có tài khoản, dùng /bank -r để tạo tài khoản 😽', threadID, messageID)
    else { 
      var userData = user.find(i => i.senderID == senderID);
      return api.sendMessage(`[ 𝐍𝐆𝐀̂𝐍 𝐇𝐀̀𝐍𝐆 ] » Số tiền bạn đang có trong tài khoản là: ${userData.money}$\n💷 Lãi suất: +${laisuat}% 𝘁𝗿𝗼𝗻𝗴 ${timeIM/60} 𝗽𝗵𝘂́𝘁`, threadID, messageID)
    }
  } 
  ...
  if (!user.find(i => i.senderID == senderID)) {
    return api.sendMessage('[ 𝐂𝐀̉𝐍𝐇 𝐁𝐀́𝐎 ] » 𝗡𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴 𝗰𝗵𝘂̛𝗮 𝘁𝗮̣𝗼 𝘁𝗵𝗲̉ 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 𝗻𝗴𝗮̂𝗻, /𝗯𝗮𝗻𝗸 -𝗿 𝗻𝗲̂́𝘂 𝗺𝘂𝗼̂́𝗻 𝘁𝗮̣𝗼 😽', threadID, messageID)
      ...
      if(balance < moneyInput) return api.sendMessage(`[ 𝐂𝐀̉𝐍𝐇 𝐁𝐀́𝐎 ] » Số dư không đủ ${moneyInput} để gửi vào Zeus Bank💰 `, threadID, messageID)
      var userData = user.find(i => i.senderID == senderID);
